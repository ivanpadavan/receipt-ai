...
const Drawer = ({
  ...
  return <DrawerPrimitive.Root
    ...
    onAnimationEnd={(open) => {
      onAnimationEnd && onAnimationEnd(open);
      if (!open) {
        setClosing(false);
        props.onCloseAnimationEnd && props.onCloseAnimationEnd()
      }
    } }
...
>(({ className, children, onPointerDownOutside, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ...
      {...props}
    >
      <div className="mx-auto mt-4 h-1.5 w-[90px] rounded-full bg-muted/80" />
      {children}
    </DrawerPrimitive.Content>
  </DrawerPortal>
))
